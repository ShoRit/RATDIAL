...
def get_arguments():
    ...
    parser.add_argument('--model_checkpoint', default='/data/datasets/models/huggingface/lmsys/vicuna-13b-v1.5-16k/', help='the huggingface model checkpoint to be used')
...
if __name__ == '__main__':
    ...
    if args.label == 'all':
        num_datapoints = len(list(dataset_strategies[args.dataset].keys()))
    
    
    str_num_datapoints = str(num_datapoints)
    print(f"Generating for {str_num_datapoints} datapoints!")
    target_dataset_hf_size = len(target_dataset_hf)
    # print(f'length of target_dataset_hf = {target_dataset_hf_size}')
    time_initial = time.time()
    generation = []
    model_config = AutoConfig.from_pretrained(args.model_checkpoint)
    generator.tokenizer.pad_token_id = model_config.eos_token_id
    for i in tqdm(range(len(target_dataset_hf))):
        prompt_here = target_dataset_hf[i]['prompt']
        output = generator(prompt_here)
    ...
    #     print(f"\n\n################################## Prompt {i + 1} ##################################\n", target_dataset_hf[i]['prompt'], end="")
    #     print(generation[i], "\n")
    
    print(f"\nDone generating with {args.model_checkpoint}! Took {minutes} minutes and {seconds} seconds.")
    
    model_cname = ''
    
    if 'Llama-2-7b-chat' in args.model_checkpoint:
        model_cname = 'Llama-2-7b-chat'
    elif 'Llama-2-13b-chat' in args.model_checkpoint:
        model_cname = 'Llama-2-13b-chat'
    elif 'flan-t5' in args.model_checkpoint:
        model_cname = 'flan-t5-base'
        
    label_to_write = args.label.replace(' ', '_')

    generations_filename = f"{args.task}_{args.dataset}_{args.info}_{model_cname}_{args.turns}_random_{args.fshot_samples}_posneg.json" 
    # generations_filename = "generations_" + str(args.dataset) + "dataset_" + str(model_checkpoint_to_write) + "model_" + str(label_to_write) + "label_" + str(args.turns) + "contextlength_" + str(args.fshot_samples) + "fewshot_" + str(args.fshot_sampling_strategy) + "strategy_" + str(args.fshot_positive_negative) + "posneg.json"
    output_folder = os.path.join(args.output_dir, "generations")
    
    if not os.path.exists(output_folder):
        os.makedirs(output_folder)
    output_filepath = os.path.join(output_folder, generations_filename)
    print(f"\nRun complete! Saving generations to {output_filepath}...")
    generation_results = []
    # print("GENERATION = ", generation)
    for i in range(len(generation)):
        datapoint_here = target_dataset_hf[i]
        original_prompt = datapoint_here['prompt']
        # print("GENERATION i = ", generation[i])
        datapoint_here['full_generated'] = generation[i][0]['generated_text']
        datapoint_here['answer_generated'] = generation[i][0]['generated_text'].replace(original_prompt, "")
        generation_results.append(datapoint_here)
        
    with open(output_filepath, 'w') as f1:
        json.dump(generation_results, f1, indent=4)
    scores = calculate_scores(generation_results)
    metrics_folder = os.path.join(args.output_dir, "metrics")
    if not os.path.exists(metrics_folder):
        os.makedirs(metrics_folder)
    with open(os.path.join(metrics_folder, generations_filename.replace('generations', 'metrics')), 'w') as f1:
