...
def encoder_LLM_info(item, tokenizer, max_len, info, turns):
    ...
    if len(rationales) > 0:
        try:
            if info == 'intention':
                dialog_text             = f"{dialog_text}{tokenizer.sep_token}{rationales[-1][0]}"
            elif info == 'assumption':
                dialog_text             = f"{dialog_text}{tokenizer.sep_token}{rationales[-1][1]}"
            elif info == 'implicit_info':
                dialog_text             = f"{dialog_text}{tokenizer.sep_token}{rationales[-1][2]}"
            elif info == 'all':
                dialog_text             = f"{dialog_text}{tokenizer.sep_token}{rationales[-1][0]}{tokenizer.sep_token}{rationales[-1][1]}{tokenizer.sep_token}{rationales[-1][2]}"
            elif info == 'utterance':
    ...
    dial_len                        = len(dial_encoding['attention_mask'][0].nonzero())     
    conversational_context          = ""
    upd_utt_len                     = dial_len
    ...
    for idx in range(0, ctx_len):
        ...
        curr_utt_len                = len(curr_utt_encoding['attention_mask'][0].nonzero()) 
    ...
    return dial_encoding, dialog_text, labels, upd_utt_len
    ...
    if len(rationales) > 0:
        try:
            if info == 'intention':
                ...
                dialog_text             = f"{dialog_text}[INTENTION]{rationales[-1][0]}[ASSUMPTION]{rationales[-1][1]}[IMPLICIT INFORMATION]{rationales[-1][2]}"
            elif info == 'utterance':
                dialog_text             = f'{dialog_text}'
        except Exception as e:
            import pdb; pdb.set_trace()
...
def chat_LLM_info(item, task_name, dataset_name, label_name, info, label_description, num_turns=5):
    utterance                           = item['utterance']
    speaker                             = item['speaker']
    context                             = item['context']
    response_lbls                       = find_label_list(task_name, dataset_name)
...
class DialogueExplanationsDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if self.instruct_flag == 1:
            dial_encoding, dial_text, labels, tok_lens = instruction_LLM_info(item, self.tokenizer, self.max_len, self.info, self.turns)
        else:
            dial_encoding, dial_text, labels, tok_lens = encoder_LLM_info(item, self.tokenizer, self.max_len, self.info, self.turns)
...
def get_data_loaders_info(
    ...
    #compute the max turns in the dataset
